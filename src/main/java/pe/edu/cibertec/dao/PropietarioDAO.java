	...
	MySqlConexion con = new MySqlConexion();
		...
		// method for fetch saved user data
				public ResultSet reportPropietario() throws SQLException, Exception {
					ResultSet rs = null;
					try {
						String sql = "SELECT idPropietario,nombres,apellidos,telefono,correo,idTipoDocumento,nroDocumento from tb_propietario";
						PreparedStatement ps = con.getConnection().prepareStatement(sql);
						rs = ps.executeQuery();
						return rs;
					} catch (Exception e) {
				...
				public int updatePropietario( String telefono, String correo, String idpropietario)
										  throws SQLException, Exception {
					
					con.getConnection().setAutoCommit(false);
					int i = 0;
					try {
						String sql = "UPDATE tb_propietario SET  telefono=?, correo=? WHERE idPropietario=?";
